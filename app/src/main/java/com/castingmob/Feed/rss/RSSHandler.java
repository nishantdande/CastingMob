...
class RSSHandler extends org.xml.sax.helpers.DefaultHandler {
  ...
  EventsRssFeedOutputItem eventsRssFeedOutputItem;
  ...
  /**
   * Setter for RSS &lt;a10:content&gt; elements inside a &lt;channel&gt; or an
   * &lt;item&gt; element. The title of the RSS feed is set only if
   * {@link #item} is {@code null}. Otherwise, the title of the RSS
   * {@link #item} is set.
   */
  private final Setter SET_ACONTENT = new ContentSetter() {
    @Override
    public void set(String content) {
      if (item != null) {
        item.setaContent(content);
      }
    }
  };

  private final Setter SET_UCONTENT = new ContentSetter() {
    @Override
    public void set(String content) {
      if (item != null) {
        item.setUcontent(content);
      }
    }
  };
